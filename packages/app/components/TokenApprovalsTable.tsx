...
export const TokenApprovalsTable = ({
  ...
  const allowanceBigNumber = ethers.BigNumber.from(allowanceData.allowance);
  const isUnlimited = allowanceBigNumber.eq(ethers.constants.MaxUint256);
                ...
                <div className="flex items-center justify-end space-x-1 w-max lg:w-auto">
                  <p className="font-semibold text-sm">
                    {isUnlimited
                      ? "Unlimited"
                      : ethers.utils
                          .formatUnits(allowanceData.allowance, 8)
                          .toString()}
                  </p>
                </div>
              </td>
